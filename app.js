...
async function navigate(screen) {
  ...
  if (screen === 'admin') { showLoading(); await loadAdminDashboard(); hideLoading(); }
  updateCartBadge();
...
function switchAdminTab(tab, el) {
  adminTab = tab;
  document.querySelectorAll('#screen-admin .cat-tab').forEach(t => t.classList.remove('active'));
  if (el) el.classList.add('active');
  renderAdminContent();
}
...
async function loadAdminDashboard() {
  ...
  const totalSales = allOrders.reduce((s, o) => s + (o.grandTotal || 0), 0);
  const today = new Date().toISOString().split('T')[0];
  const todayOrders = allOrders.filter(o => o.createdAt?.startsWith(today));

  document.getElementById('admin-kpi-devices').querySelector('.value').textContent = '1 device';
  document.getElementById('admin-kpi-online').querySelector('.value').textContent = navigator.onLine ? 'Online' : 'Offline';
  document.getElementById('admin-kpi-orders').querySelector('.value').textContent = allOrders.length;
  document.getElementById('admin-kpi-sales').querySelector('.value').textContent = '$' + totalSales.toLocaleString();

  window._adminData = { allOrders, allUsers, allCustomers, sync, totalSales, todayOrders };
  renderAdminContent();
}
...
function renderAdminContent() {
  ...
  if (!data) return;
  if (adminTab === 'devices') renderAdminDevices(data);
  if (adminTab === 'orders') renderAdminOrdersLocal(data);
  if (adminTab === 'users') renderAdminUsersLocal(data);
  if (adminTab === 'customers') renderAdminCustomersLocal(data);
}

function renderAdminDevices(data) {
  const html = `
    <div class="card card-glow" style="margin-bottom:8px">
      <div class="flex justify-between items-start">
        <div>
          <div style="font-weight:600;font-size:14px">Mineazy Mobile POS</div>
          <div class="mono-sm text-muted">${DEVICE_ID.slice(0, 16)}...</div>
        </div>
        <span class="badge ${navigator.onLine?'badge-green':'badge-red'}">${navigator.onLine?'ONLINE':'OFFLINE'}</span>
      </div>
      <div class="flex justify-between" style="margin-top:8px;font-size:12px">
        <span class="text-muted">Orders: <b>${data.allOrders.length}</b></span>
        <span class="text-muted">Sales: <b>$${data.totalSales.toLocaleString()}</b></span>
        <span class="text-muted">Pending: <b>${data.sync.length}</b></span>
      </div>
      <div class="text-muted" style="font-size:10px;margin-top:4px">Users: ${data.allUsers.length} · Customers: ${data.allCustomers.length}</div>
    </div>
  `;
  document.getElementById('admin-tab-content').innerHTML = html;
}

function renderAdminOrdersLocal(data) {
  const orders = data.allOrders.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  const html = orders.length === 0
    ? '<div class="empty-state"><span class="material-symbols-outlined icon">receipt_long</span><p>No orders yet</p></div>'
    : orders.map(o => `
      <div class="card card-glow" style="margin-bottom:8px">
        <div class="flex justify-between items-start">
          <div>
            ...
            <div class="text-muted" style="font-size:10px">By: ${o.salesRepName||o.userId||'-'} · ${timeAgo(o.createdAt)}</div>
          </div>
          <div style="text-align:right">
            <div style="font-size:16px;font-weight:700;color:var(--primary-dim)">$${(o.grandTotal||0).toLocaleString()}</div>
            <span class="badge ${o.status==='confirmed'?'badge-green':'badge-yellow'}">${o.status}</span>
          </div>
        </div>
        <div class="text-muted" style="font-size:10px;margin-top:4px">${(o.items||[]).length} products · ${(o.items||[]).reduce((s,i)=>s+(i.qty||i.quantity||0),0)} items</div>
      </div>
    `).join('');
  document.getElementById('admin-tab-content').innerHTML = html;
}

function renderAdminUsersLocal(data) {
  const users = data.allUsers;
  const html = users.length === 0
    ? '<div class="empty-state"><span class="material-symbols-outlined icon">people</span><p>No users yet</p><p class="text-muted" style="font-size:11px">Sign up to register users</p></div>'
    : users.map(u => {
        const userOrders = data.allOrders.filter(o => o.userId === u.id);
        const userSales = userOrders.reduce((s, o) => s + (o.grandTotal || 0), 0);
        return `
      <div class="card card-glow" style="margin-bottom:8px">
        <div class="flex justify-between items-start">
          <div>
            <div style="font-weight:600">${u.displayName||u.id}</div>
            <div class="mono-sm text-muted">@${u.id} · ${u.role||'sales_rep'}</div>
          </div>
          <span class="badge badge-green">ACTIVE</span>
        </div>
        <div class="flex justify-between" style="margin-top:6px;font-size:11px">
          <span class="text-muted">Orders: <b>${userOrders.length}</b></span>
          <span class="text-muted">Sales: <b>$${userSales.toLocaleString()}</b></span>
        </div>
      </div>
    `}).join('');
  document.getElementById('admin-tab-content').innerHTML = html;
...
function renderAdminCustomersLocal(data) {
  const customers = data.allCustomers;
  const html = customers.length === 0
    ? '<div class="empty-state"><span class="material-symbols-outlined icon">group</span><p>No customers yet</p></div>'
    : customers.map(c => `
      <div class="card card-glow" style="margin-bottom:8px">
        <div class="flex justify-between items-start">
          <div style="flex:1">
            <div style="font-weight:600">${c.name}</div>
            <div class="mono-sm text-muted">${c.company||''} · ${c.phone||''} · ${c.email||''}</div>
          </div>
          <div style="text-align:right">
            <div class="badge badge-green" style="font-size:10px">${c.orderCount||0} orders</div>
          </div>
        </div>
      </div>
    `).join('');
